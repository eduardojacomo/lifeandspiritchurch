<template>
  <div class="tab-content active">
    <section id="upload-section" v-if="!form.preview && !props.editData">
    ...
    <section id="edit-image-section" v-else>
      <div class="config-section active">
        <h2 style="margin-bottom: 2rem; font-size: 1.8rem;">
          {{ props.editData ? 'Editar Integrante' : 'Configurar Novo Integrante' }}
        </h2>
          ...
          <div class="image-config-item">
            ...
            <div class="previews-column">
              <div class="main-preview">
                <label>🖥️ Foto Perfil</label>
                <div class="img-wrapper">
                  <img :src="form.preview" alt="Preview">
                </div>
              </div>
              <p class="file-name" v-if="form.nome">{{ form.nome.toLowerCase() }}_profile.webp</p>
              
              <div class="action-buttons">
                <button class="btn-remove" @click="resetForm">
                  Trocar Foto
                </button>
              </div>
              ...
              <div class="form-group">
                ...
                <div class="device-toggle">
                    <div 
                    class="device-option" 
                    :class="{ selected: form.isMainAbout }"
                    @click="form.isMainAbout = !form.isMainAbout"
                    >
                    <input type="checkbox" v-model="form.isMainAbout">
                    <span>Destaque no "Sobre Nós" (Foto e Texto Detalhado)</span>
                    </div>
                </div>
                <p style="color: rgba(255, 255, 255, 0.4); font-size: 0.8rem; margin-top: 0.5rem;">
                    * Ao marcar esta opção, o integrante anterior deixará de ser o destaque automaticamente.
                </p>
                </div>
            </div>
            <div class="config-form">
              ...
              <div class="column">
                <div class="form-group form-half">
                  ...
                  <input type="text" v-model="form.cargo.pt" placeholder="Ex: Pastor">
                </div>
                <div class="form-group form-half">
                  <label>Cargo (EN)</label>
                  <input type="text" v-model="form.cargo.en" placeholder="Ex: Pastor">
                </div>
              </div>

              <div class="column">
                <div class="form-group form-half">
                  <label>Sobre (PT)</label>
                  <textarea v-model="form.about.pt" placeholder="Biografia em português"></textarea>
                </div>
                <div class="form-group form-half">
                  <label>Sobre (EN)</label>
                  <textarea v-model="form.about.en" placeholder="Biography in English"></textarea>
                </div>
              </div>
              ...
              <div class="column">
                <div class="form-group form-half">
                  ...
                  <input type="text" v-model="form.ministerio" placeholder="Ex: Louvor / Kids">
                </div>
        ...
        <button 
          ...
          :disabled="isLoading"
        >
          {{ isLoading ? 'Processando...' : (props.editData ? 'Atualizar Integrante' : 'Salvar no Time') }}
        </button>
      </div>
    </section>
...
const { saveMember, isLoading } = useTeamRepository();
...
const form = reactive({
  ...
  cargo: { pt: '', en: '' },
  about: { pt: '', en: '' },
  preview: null, // Data URL para o upload
  url: '',        // URL existente no Firestore
  isMainAbout: false
});
...
// Lógica de Arquivos (Drag & Drop + Select)
...
async function handleSave() {
  try {
    // Mapeia para o padrão esperado pelo seu saveMember
    const payload = {
      ...form,
      newImagePreview: form.preview.startsWith('data:') ? form.preview : null
    };
